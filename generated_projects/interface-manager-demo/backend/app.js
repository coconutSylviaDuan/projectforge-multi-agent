    ...
    try {
        ...
        let html = `
            <div class="row mb-3">
                ...
                <div class="col-md-3">
                    <select class="form-select" onchange="state.filterMethod=this.value; state.page=1; renderInterfacesPage()">
...
async function renderDetailPage(interfaceId) {
    ...
    try {
        ...
        renderInterfaceDetail(iface, tab);
    } catch (e) {
...
function renderInterfaceDetail(iface, activeTab) {
    ...
    let html = `
        <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="#/interfaces">接口目录</a></li><li class="breadcrumb-item active">${iface.name}</li></ol></nav>
        <div class="card">
            <div class="card-body">
    ...
    `;
    render(html + '<div class="text-center">加载中...</div></div>');
    // 加载对应tab内容
    loadTabContent(iface.id, activeTab);
}
...
async function loadTabContent(interfaceId, tab) {
    try {
        ...
        if (tab === 'mock') {
            ...
            contentHtml = `
                <form id="mockForm" onsubmit="saveMockConfig(event, ${interfaceId})">
                    <div class="mb-3 form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="enabledSwitch" ${config.enabled ? 'checked' : ''}>
                        <label class="form-check-label" for="enabledSwitch">启用Mock</label>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">状态码</label>
                        <input type="number" class="form-control" id="statusCode" value="${config.status_code}">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="delayMs" value="${config.delay_ms || 0}">
                    </div>
            ...
            if (data.items.length === 0) {
                ...
                for (const log of data.items) {
                    tableHtml += `<tr><td>${log.field}</td><td title="${log.old_value}">${log.old_value.substring(0,50)}${log.old_value.length > 50 ? '...' : ''}</td><td title="${log.new_value}">${log.new_value.substring(0,50)}${log.new_value.length > 50 ? '...' : ''}</td><td>${formatDate(log.changed_at)}</td></tr>`;
                }
        ...
        document.getElementById('tabContent').innerHTML = `<div class="alert alert-danger">加载失败: ${e.message}</div>`;
    }
}
...
async function saveIntegration(event, interfaceId) {
    ...
    try {
        ...
        // 更新badge
        const badgeClass = { '未开始': 'secondary', '联调中': 'primary', '已完成': 'success' }[newStatus.status];
        document.querySelector('.badge').className = `badge bg-${badgeClass} status-badge`;
        document.querySelector('.badge').textContent = newStatus.status;
    } catch (e) {
